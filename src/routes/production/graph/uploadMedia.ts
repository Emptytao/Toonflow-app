...
import { getEpisodeId } from "@/lib/productionGraph";
...
export default router.post(
  ...
  async (req, res) => {
    try {
      const { projectId, base64Data } = req.body;
      const episodeId = getEpisodeId(req.body);
      const extInfo = getExtFromBase64(base64Data);
      if (!extInfo) {
        return res.status(400).send(error("不支持的媒体类型"));
      }
      const savePath = `/${projectId}/productionGraph/${episodeId}/uploads/${uuidv4()}.${extInfo.ext}`;
