...
description: >-
  通用分镜表技法参考。
  涵盖拆镜原则、镜头合并、连续性铁律、字段填写与转场边界等分镜设计通用技法，只服务 storyboard table。
---
# 分镜表通用技法

本文档为分镜表阶段的通用技法参考，适用于所有需要构建分镜表的 Agent 场景。
...
- 本文档只服务分镜表阶段。
- 只处理拆镜、镜头任务、动作连续性、情绪承接、字段落地。
- 不承担最终视频提示词的戏剧化润色。
- 不写任何光影、色调、色温、明暗词。
- 不写任何 BGM、配乐、音乐节奏、乐器质感。
...
- 强情绪优先落在反应镜、停顿、呼吸、手部细节或姿态变化上，不只靠说话人正面输出。
- 动作优先写成连续动作链，不只写最终状态。
- 一个镜头内只保留一个主要叙事任务和一个主要动作链，多余内容拆到相邻镜头。
- 相邻镜头必须明确“承接了上一镜什么状态，新增了什么信息，抛给下一镜什么问题”。
...
> **光影/色调边界**：description 可包含必要的光线、色彩、氛围信息，但不能用它们替代主体动作、状态变化和空间关系。特殊光照需求优先通过对应**场景衍生**（夜景版/雨天版/火光版）加强一致性；最终视频 prompt 仍可按模型模板自行处理光影质感。
