...
2. **语义等价转换**：将分镜表字段转换为提示词时，只改变表达形式（中↔英、散文↔关键词、叙事语言↔视觉描述），**不改变语义**。例：分镜表写"建筑空间廊柱密集" → 提示词必须体现该空间结构，不可替换为"华丽建筑"等不同语义
...
| 情绪 | 情绪面容词 | 情绪基调是否一致 |
| 图像氛围 | 提示词正文【光影】段 | 仅由风格专属技法、场景资产状态或衍生场景生成，不要求分镜表提供光影字段 |
...
| 图像氛围 | 可独立成【光影】段，但只能根据风格专属技法、场景资产状态或衍生场景推导；不得要求分镜表存在「光影氛围」字段 |
| 台词 | 不进入提示词，不输出 |
| 音效 | 不进入提示词，不输出 |
...
【光影】→ 由风格专属技法、场景资产状态或衍生场景生成的图像氛围（独立成段，避免被风格词挤压）
【风格】→ 风格锚定词 + 画质锁定词 + 禁止项声明（辅助修饰，简短）
```

> **篇幅分配原则**：【画面】段为信息密度最高、篇幅最长的段落，须完整承载分镜表「画面描述」的所有视觉元素；【光影】段次之，只承载风格与场景资产可推导出的图像氛围；【风格】段最短，仅放必要的风格锚定词与画质锁定词。三段顺序不可颠倒、长度不可倒置——若风格词篇幅超过画面段，即为失败产出。
...
| 情绪 | 局促与笃定并存 |
| 图像氛围来源 | 使用风格专属技法与「某场景出口」资产状态推导，不从分镜表读取光影字段 |
