...
- **审核规则**：阶段1（导演规划）与阶段4（构建分镜表）执行完毕后，均自动派发监督层
...
阶段4完成后、派发阶段5之前，根据**视频模型身份**决定写入模式：

| 视频模型 | 决策层操作 |
|----------|-----------|
| Seedance 2.0 | 使用 **"纯文本多参模式"** 派发给执行层 |
| 非 Seedance 2.0 | 无需询问用户，直接以 **"首位帧模式"** 派发给执行层 |
...
- 分组累计时长不得超过 15 秒
- 派发执行层时必须在指令开头明确携带写入模式前缀（纯文本多参模式 / 首位帧模式）
...
run_sub_agent_supervision(
  prompt: "请执行{审核对象}。审核对象：{审核对象}。请审核【{阶段名}】的产出物。审核维度：{维度列表}"
)
```
