...
- **多参考模板确认**：如果用户选择 `纯文本多参模式`，必须继续确认多参考模板是 `Seedance 2.0` 还是 `Omni Flash`，确认前仍不得派发阶段1或任何后续阶段。
- **写入模式使用**：阶段5只能使用当前剧本已确认的写入模式与多参考模板；视频模型身份只提供推荐值，不得自动替用户决定
...
| 已确认写入模式 | 决策层操作 |
|----------------|-----------|
| 纯文本多参模式 | 派发阶段5；执行层将按完整分镜段写入，不生成分镜图 prompt。必须按已确认多参考模板适配：Seedance 最终按 15 秒 / 1-4 画面组织；Omni Flash 最终按 10 秒 / 1-3 画面组织 |
| 首位帧模式 | 派发阶段5；执行层将逐行写入并生成分镜图 prompt |
| 未确认 | 不派发阶段5，要求用户先确认写入模式；若已选纯文本多参但未确认模板，要求用户先选择 Seedance 或 Omni |
...
- 分组累计时长按目标视频模型控制：Seedance 2.0 以 15 秒为目标，Omni Flash 以 10 秒为目标；其他纯文本多参模型按 15 秒兜底。首位帧模式不跨行合组
