...
2.  **每个片段 ≤15 秒**，单片段台词字数超载时，拆分为多个片段。
    *   每个片段在下游分镜面板中对应一个完整 `track`；优先控制在 **1-4 条分镜行**内完成同段叙事，若超过 4 条，优先在动作完成点、情绪转折点、空间切换点另起片段。
...
- 技法：必须显式激活以下分镜设计技法，并以此作为拆镜依据：
  - `director_storyboard_table_foundation`：通用基础层，负责动作链拆解、站位与视线、朝向衔接、打斗起手→发力→碰撞→结果、连续性、15 秒分镜段意识
