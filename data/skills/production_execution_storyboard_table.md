...
2.  **片段时长按已确认多参考模板控制**，单片段台词字数超载时，拆分为多个片段。
    *   纯文本多参模式 + Seedance 2.0：每个片段 ≤15 秒，优先控制在 **1-4 条分镜行**内完成同段叙事。
    *   纯文本多参模式 + Omni Flash：每个片段 ≤10 秒，优先控制在 **1-3 条分镜行**内完成同段叙事。
    *   首位帧模式：按单镜/单行写入，不为了多参合段强行压组。
    *   每个片段在下游分镜面板中对应一个完整 `track`；若超过目标行数，优先在动作完成点、情绪转折点、空间切换点另起片段。
...
2.  **切片段**：沿叙事顺序，在情绪转折点/动作段落/说话人切换处下刀；Seedance 按 ≤15 秒切段，Omni 按 ≤10 秒切段，首位帧模式按单镜节奏拆分。
...
- 技法：必须显式激活以下分镜设计技法，并以此作为拆镜依据：
  - `director_storyboard_table_foundation`：通用基础层，负责动作链拆解、站位与视线、朝向衔接、打斗起手→发力→碰撞→结果、连续性、按已确认模板控制分镜段时长
...
7.  **禁配乐**：任何字段不写 BGM / 音乐 / 配乐 / 乐器烘托。光线、氛围、色彩可在有明确画面必要时自然描述，但不得替代剧情动作、人物状态和空间关系。
