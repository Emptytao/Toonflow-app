...
2. 确定分组（track）与时长规则：
   - `track` 表示**一个完整分镜段**，同一 `track` 内的多条分镜应共同服务同一段连续叙事
   - **纯文本多参模式 / 分镜图辅助多参模式**：同组内分镜 `duration` 累计时长不得超过 15 秒
   - **软约束**：同一 `track` 优先控制在 `1-4` 条分镜行；若超过 4 行，优先在动作完成点、情绪转折点、空间切换点开启新 `track`
...
5. **生成视频描述（videoDesc）**（所有模式均需）：根据 `storyboardTable` 对应行的完整分镜数据（画面描述、场景、关联资产名称、时长、景别、运镜、角色动作、朝向、空间关系、情绪、台词、音效、关联资产ID），将该行信息整合为一段结构化的视频描述文本，填入 `videoDesc` 字段。台词字段必须保留原有类型边界，明确区分普通对白 / 内心独白 `OS` / 画外音 `VO`；音效仅承载环境音与动作音。**禁止包含任何光影/色温/明暗/色调描述**——光影由视频模型从场景图自动推导
6. **生成提示词（prompt）并忠实性校验**（纯文本多参模式跳过此步）：逐行读取 `storyboardTable` 对应行的「画面描述」「场景」「景别」「角色动作」「朝向」「空间关系」「情绪」「台词」「音效」字段，严格按照步骤1已加载技法中的「分镜表内容忠实性原则」和「解析映射规则」将各字段映射为提示词各段落。**提示词正文不得包含光影/色温/明暗/色调描述**——这些由场景图参考自动承担。**生成每条提示词后须立即逐字段比对分镜表原始内容**，确认：① 画面描述中的所有视觉主体和空间关系均已完整保留在提示词正文中；② 情绪基调与分镜表一致；③ 提示词中无光影/色调相关词汇；④ 景别匹配；⑤ 角色动作语义一致（仅形式按首帧原则转换，不替换为不同动作）；⑥ 角色朝向与步骤3基准表一致，且 prompt 中已显式标注朝向方位词；⑦ 台词类型未被误改，普通对白 / 内心OS / 画外音VO 保持原边界；⑧ 音效仅保留具体可感知声源。校验不通过须修正后再进入下一步
...
- **track 语义固定**（纯文本多参模式 / 分镜图辅助多参模式）：同一 `track` 代表一个完整分镜段，应优先在 `1-4` 条分镜内完成同段叙事；若超过 4 条，优先在叙事边界处分新 `track`
...
- **台词类型保留**（所有模式）：若分镜表原始信息区分普通对白 / 内心独白 `OS` / 画外音 `VO`，则 `videoDesc` 与 `prompt` 必须保留这一类型边界，不得互相改写
