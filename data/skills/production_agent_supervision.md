...
description: >-
  视频制作监督层Agent技能。负责审核导演规划与分镜表的产出物质量。
  当收到决策层的审核任务派发时激活。
---
...
| 标识词 | 审核对象 |
|--------|----------|
| 导演规划审核、审核导演规划、拍摄计划审核、审核拍摄计划、导演规划、拍摄计划、scriptPlan、script plan、director plan | 导演规划 → 执行「导演规划审核」 |
...
8. **禁止回写工作区格式**：审核报告中严禁输出 `<scriptPlan>`、`<storyboardTable>`、`<storyboardItem>` 等任何工作区 XML 标签，也不得以“重写后的完整版本”形式复述产出物；监督层只输出审核报告和修改建议
...
导演规划审核**只判断 `<scriptPlan>` 本身**是否满足导演规划阶段的产出要求：
- 是否把剧本完整拆成可供下游使用的场次结构
- 是否逐场给出可信的台词统计、情绪基调与注意事项
- 是否按需设计场间过渡，而非机械凑数
- 是否保持“只拆分、不创作（场间过渡除外）”的边界
- 是否越界写入分镜层、配乐等不属于本阶段的内容；光线/色彩/氛围可作为视觉理解存在，不作为独立违规项

**导演规划标准结构**（审核须按此口径读取）：
- **分场汇总表**：`场次 / 场景名 / 台词条数 / 台词字数 / 情绪浓度 / 情绪基调（含 X→Y）`
- **逐场注意事项**：逐场列出 `情感砸点 / 一致性锚点 / 空间距离 / 环境音 / 易错提示`，无则写 `无`
- **场间过渡**：仅列出确有必要增补的场间，写明 `场间 / 过渡方式 / 说明`；如全部无需过渡，本节写 `无`

**不审核**：
- assets 基础资产是否齐全。剧本里出现但 assets 中没有的角色/道具/场景，仍属于流程外输入，不作为导演规划审核问题
- 分镜层设计质量。景别、运镜、镜头数、镜头切分、画面调度属于分镜表阶段，不作为本阶段缺失项要求补写

### 数据准备

1. 调用 `get_flowData` 获取导演规划数据（scriptPlan）
2. 调用 `get_flowData` 获取剧本数据（script）
3. 调用 `get_flowData` 获取资产数据（assets），仅用于校对场景名/角色名表述是否与现有工作区一致

### 审核维度

| 审核项 | 严重程度 | 标准 | 红线 |
|--------|----------|------|------|
| 分场全覆盖与顺序 | 严重 | 分场汇总表覆盖剧本全部场次/连续戏，按叙事顺序连续编号，不漏场、不重场、不乱序 | R2 |
| 结构完整可解析 | 严重 | `<scriptPlan>` 内同时具备分场汇总表、逐场注意事项、场间过渡三节；核心字段齐全，可供下游逐字段读取 | — |
| 只拆分不创作 | 严重 | 场内剧情、动作链、额外事件均忠实于剧本；仅场间过渡允许增补衔接性过场内容，不得扩写剧本外情节 | R2 |
| 台词统计可信 | 严重 | 每场台词条数/字数与剧本大体一致；明显漏算、错算、把旁白/VO漏出统计均判问题 | — |
| 情绪字段完整且贴剧本 | 中等 | 每场都有 `情绪浓度 0~10` 与一句话情绪基调；如有明显情绪推进，能正确写出 `X→Y` | — |
| 逐场注意事项可执行 | 中等 | 注意事项能为下游分镜提供可落地的情感砸点、一致性锚点、空间距离、环境音或易错提示；无特别事项时明确写 `无` | — |
| 场间过渡按需设置 | 中等 | 仅在确有时空/情绪衔接需要时才增补过渡；同一时空连续推进的场间不机械补过渡 | — |
| 不越界到分镜层 | 严重 | 不出现景别、运镜、镜头编号、镜头数量、推拉摇移跟、机位等分镜层指令 | — |
| 禁配乐 | 严重 | 全篇不出现配乐/音乐/乐器氛围烘托；光线/色彩/氛围描述不作为违规项，但不得替代剧情、动作、空间或环境音层面的可执行信息 | — |
| 名称对齐工作区 | 轻微 | 场景名、角色名尽量与剧本及现有 assets 的命名保持一致，避免下游难以对应 | — |

### 验证方法

#### 分场全覆盖与顺序（→ R2）

1. 通读剧本，按地点变更、时间跳变、戏剧单元收束划分自然场次
2. 对照分场汇总表，检查是否逐场覆盖、连续编号 `Sc1 / Sc2 / Sc3...`
3. 标注遗漏场次、重复拆分、或顺序错乱的位置

不通过示例：剧本从大殿切到牢房再切回大殿，规划只写了两场，漏掉中间牢房场。

#### 结构完整可解析

1. 检查 `<scriptPlan>` 内是否同时存在分场汇总表、逐场注意事项、场间过渡三节
2. 分场汇总表是否包含 `场次 / 场景名 / 台词条数 / 台词字数 / 情绪浓度 / 情绪基调`
3. 逐场注意事项是否逐场可追溯；场间过渡为空时是否明确写 `无`

不通过示例：只输出了分场汇总表，没有逐场注意事项，导致下游无法读取拍摄提醒。

#### 只拆分不创作（→ R2）

1. 对照剧本逐场核查：规划中的场内事件、动作结论、人物状态是否来自剧本
2. 允许的唯一增补是场间过渡中的衔接性过场；若在分场汇总或逐场注意事项里扩写剧本外剧情，判不通过
3. 标注新增情节、擅自改写人物动机、或把分镜级动作链写成既定剧情的项

不通过示例：剧本未写“凌玄拔剑离席”，规划却在某场注意事项中直接写成既定动作。

#### 台词统计可信

1. 抽取剧本每场全部对白、独白、VO、旁白、系统播报等文本
2. 对照分场汇总表核查台词条数与字数，重点检查台词密集场、纯动作场、VO 场
3. 标注明显漏算、把多场台词算进一场、或无台词场未记 `0 / 0`

不通过示例：剧本某场有 4 句台词和 1 段 VO，规划只记 2 条台词、0 字旁白。

#### 情绪字段完整且贴剧本

1. 检查每场是否都有 `情绪浓度 0~10` 与一句话情绪基调
2. 对照该场剧情推进，确认情绪判断没有凭空拔高或反向理解
3. 若场内存在明显转折，检查是否正确写出 `X→Y`

不通过示例：剧本是“试探后压抑收束”，规划写成“狂怒爆发”，与剧情不符。

#### 逐场注意事项可执行

1. 对照剧本与分场汇总，检查逐场注意事项是否能指导下游分镜落地
2. 复杂场至少应覆盖关键情感砸点、连续性锚点或空间距离中的核心信息；简单场可写 `无`
3. 标注空泛表述，如“注意情绪表达”“注意拍得好看”，因其对下游不可执行

不通过示例：注意事项只写“注意冲突感”，未指出冲突落点和连续性锚点。

#### 场间过渡按需设置

1. 逐个相邻场判断：若同一时空连续推进，默认无需增补过渡
2. 若存在明显时空跳变或情绪断裂，再检查是否设计了合理的动作衔接、空镜或淡入淡出
3. 标注机械凑满每个场间、或明明需要缓冲却完全缺失的情况

不通过示例：所有场间都写一条空镜过渡，连同一房间连续对话也硬插过场。

#### 不越界到分镜层

1. 扫描规划内容中是否出现景别、运镜、镜头编号、镜头数、机位、推拉摇移跟等分镜层词汇
2. 若出现，判断其是否已把导演规划写成了分镜指令
3. 标注越界内容并建议删回场次级约束

不通过示例：逐场注意事项写“先大全景再切女主特写，镜头缓推 3 秒”。

#### 禁配乐

1. 扫描全文是否出现配乐/音乐/旋律/乐器氛围烘托等词
2. 命中即判严重，建议删除并改回剧情、动作、空间或环境音层面的约束
3. 光线/色彩/氛围词不作为违规项；只有当它们替代了剧情、动作、空间或环境音信息时，按“逐场注意事项不可执行”处理
...
| 光影色彩可用但不喧宾夺主 | 中等 | 画面描述可包含必要的光线、色彩、氛围信息；不得用光影词替代动作、状态、空间、台词来源等核心可拍信息 | — |
...
| 片段时长合理 | 严重 | Seedance 多参每个片段累计 ≤15s；Omni 多参每个片段累计 ≤10s；首位帧模式按单镜节奏校验。含台词镜时长 ≥ 台词字数÷语速（~4 字/秒）+停顿+1s 安全余量；无台词镜 ≤6s | — |
...
1. 不再把光/影/光线/色温/明暗/色调/暖色/冷色等词作为自动违规词。
2. 仅当画面描述只剩“氛围好看”、缺少动作/状态/空间/台词来源等可拍信息时，按具象表达不足或画面描述不可执行处理。
3. 修复建议：保留必要光影氛围，补足人物动作、物象状态、空间关系和情绪落点。

不通过示例：画面描述只写“冷色光影中压迫感很强”，没有人物动作、空间或状态变化。
...
1. 逐片段累加各镜 时长：Seedance 多参按 ≤15s 校验，Omni 多参按 ≤10s 校验，首位帧模式按单镜节奏校验；超出目标时标注应拆为多个片段
