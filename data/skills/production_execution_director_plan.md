...
同轮调用 `get_flowData("script")` 与 `get_flowData("assets")`。同时显式激活 `director_planning_foundation`、`director_planning_narrative`、`director_planning_style` 三类导演规划技法。
...
- **读取**：`get_flowData("script")` / `get_flowData("assets")` —— **整个任务仅在第 1 步各用一次**；此后严禁再调用任何读取类工具。
- **技法**：必须显式激活 `director_planning_foundation`、`director_planning_narrative`、`director_planning_style`。其中：
  - `director_planning_foundation` = 通用基础层，负责剧情保真、节奏骨架、台词/内心独白/声音边界、镜头服务剧情原则
  - `director_planning_narrative` = 当前题材叙事层，负责题材气质与段落调性
  - `director_planning_style` = 当前风格层，负责镜头表达方式与视觉组织方向
  - 三者冲突时，先守基础层边界，再服从题材层与风格层
...
- **不规划配乐**：配乐不在本流水线产物内；全篇不得规划音乐/配乐/乐器烘托。光线、色彩、氛围可作为视觉理解存在，但不得替代剧情、动作、空间和情绪锚点。
...
- 过渡服务情绪节奏，**不规划配乐**；光线、色彩、氛围可作为视觉理解存在，但不得替代剧情、动作、空间和情绪锚点。
...
1. **基础层必须生效**：第 1 步读取后，必须显式激活 `director_planning_foundation`、`director_planning_narrative`、`director_planning_style`；冲突时先守基础层边界，再服从题材层与风格层。
...
8. **禁配乐**：全篇任何字段不出现音乐/配乐/乐器烘托。光线、色彩、氛围可作为视觉理解存在，但不得替代剧情、动作、空间和情绪锚点。
9. **XML 一次性完整**：`<scriptPlan>…</scriptPlan>` 标签及全部内容一次性输出，禁止拆分为多次 XML 输出。
10. **不越权用工具**：全程只用「第 1 步读取」+「写出 scriptPlan」两类动作，未调用任何资产写入或其他阶段的工具。
